...
#Missing= 11,689
#Reduced number of observations (countries*indicators) to 37,022
...
cntryind$missing <- mapply(function(x) { if (x>=95) {return (1)} else {return (NA)}}, x=cntryind$x)
...
#NEXT STEPS: Separate each indicator into a seperate row and then impute missing using HMisc
indicators$X2012Impute.count <-mapply(function(x) {return(is.finite(x))},x=indicators$X2012Impute)
